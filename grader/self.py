...
def check_page_replacement() -> List[Check]:
    return check_mipster_execution('memory-stress-basic.c', 42,
                                   'basic memory stress test with page replacement works', 
                                   timeout=60) + \
        check_mipster_execution('lru-verification.c', 42,
                                'LRU algorithm correctly identifies least recently used pages',
                                timeout=60) + \
        check_mipster_execution('dirty-page-swapping.c', 42,
                                'dirty pages are correctly preserved during swapping',
                                timeout=120) + \
        check_mipster_execution('page-access-pattern.c', 42,
                                'page access patterns correctly update LRU information',
                                timeout=60) + \
        check_mipster_execution('swap-space-limit.c', 42,
                                'system handles swap space limits gracefully',
                                timeout=120) + \
        check_mipster_execution('page-replacement-stats.c', 42,
                                'page replacement statistics are tracked correctly',
                                timeout=60) + \
        check_execution('./selfie -c <assignment>memory-stress-basic.c -m 4',
                        'page replacement system initializes correctly',
                        success_criteria=lambda code, out: 'page replacement' in out.lower() or code == 42) + \
        check_execution('./selfie -c <assignment>dirty-page-swapping.c -m 2',
                        'page replacement handles memory pressure correctly',
                        success_criteria=lambda code, out: code == 42 or 'page replacement' in out.lower())
...
assignment_page_replacement = Assignment('page-replacement', 'Systems', 'memory-management',
            REPO_BLOB_BASE_URI + 'grader/systems-assignments.md#assignment-page-replacement',
            check_page_replacement, parent = assignment_processes)
