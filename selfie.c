...
uint64_t* lock_holder=(uint64_t *)0; // pointer to lock holder, used for deadlock detection
...
// LRU functions
void init_lru(uint64_t max_pages);
uint64_t find_lru_entry(uint64_t *context, uint64_t page);
void update_lru_entry(uint64_t *context, uint64_t page, uint64_t frame);
uint64_t get_lru_victim_frame(uint64_t *victim_context, uint64_t *victim_page);
void remove_lru_entry(uint64_t *context, uint64_t page);
...
void init_memory(uint64_t megabytes)
{
  ...
  // iInicializar LRU con un maximo razonables
  init_lru(PHYSICALMEMORYSIZE / PAGESIZE / 10);
}
...
// | 34 | priority        | priority of process
// | 35 | prev_priority   | previous priority of process
...
uint64_t get_priority(uint64_t *context) {
...
void set_priority(uint64_t *context, uint64_t new_priority){*(context+34)=new_priority;}
void set_prev_priority(uint64_t *context, uint64_t new_prev_priority){*(context+35)=new_prev_priority;}
...
void unmap_page(uint64_t *context, uint64_t page);
...
// LRU page replacement variables
uint64_t *lru_pages = (uint64_t *)0;        
uint64_t *lru_frames = (uint64_t *)0;       
uint64_t *lru_contexts = (uint64_t *)0;     
uint64_t lru_size = 0;                      
uint64_t lru_max_size = 0;                  
...
uint64_t *extract_best_from_list(uint64_t *best, uint64_t *used_cntx){

  if (get_next_context(best) != (uint64_t *)0)
    set_prev_context(get_next_context(best), get_prev_context(best));

  if (get_prev_context(best) != (uint64_t *)0) {
    set_next_context(get_prev_context(best), get_next_context(best));
    set_prev_context(best, (uint64_t *)0);
  } else {
    used_cntx = get_next_context(best);
  }

  set_next_context(best, (uint64_t *)0);
  return used_cntx;
}

void helper_use_context(uint64_t *context) {
...
uint64_t *helper_find_context(uint64_t *context, uint64_t* list) {
  uint64_t *current;
...
void sort_context(){
  ...
  while(used_contexts!=(uint64_t*)0){
    ...
    //Conseguir el contexto de mayor prioridad:
    while(current!= (uint64_t*)0){
      if(get_priority(current)>best_priority){
        best=current;
        best_priority= get_priority(current);
      }
      current=get_next_context(current);
    }
    //best ya tendria el contexto de mayor prioridad
    used_contexts=extract_best_from_list(best, used_contexts);

    if(new_list==(uint64_t*)0){
      new_list= best;
      set_next_context(best,(uint64_t*)0);
      set_prev_context(best,(uint64_t*)0);}
    else{
      current=new_list;
      while(get_next_context(current)!=(uint64_t*)0){
        current=get_next_context(current);
      }

      set_next_context(current, best);
      set_prev_context(best, current);
      set_next_context(best, (uint64_t*)0);
    }
  }
  used_contexts=new_list;
}

uint64_t* find_context_by_id(uint64_t pid) {
  uint64_t* current;
  current = used_contexts;
  while (current != (uint64_t*)0) {
      if (get_id(current) == pid) {
          return current;
      }
      current = get_next_context(current);
  }
...
void priority_reset(uint64_t* context){
  uint64_t prev_priority;

  if(context==(uint64_t*)0){
    return;
  }

  prev_priority=get_prev_priority(context);
  if(prev_priority!=0){
    set_priority(context,prev_priority);
    set_prev_priority(context,0);
    sort_context();
  }  
}

void donate_priority(uint64_t* waiting_context,uint64_t* lock_holder_context){
  uint64_t waiting_priority;
  uint64_t lock_holder_priority;

  if (waiting_context == (uint64_t*)0) {
      return;
  }
  if (lock_holder_context == (uint64_t*)0) {
      return;
  }

  waiting_priority=get_priority(waiting_context);
  lock_holder_priority=get_priority(lock_holder_context);

  if(waiting_priority>lock_holder_priority){
    //Guardo mi prioridad caso no se halla guardado antes(Este comentario no es ia profe :))
    if(get_prev_priority(lock_holder_context)==0){
      set_prev_priority(lock_holder_context,lock_holder_priority);
    }

    set_priority(lock_holder_context,waiting_priority);
    sort_context();
  }
}
...
void implement_fork(uint64_t *context)
{
  ...
  // AGREGAR: Heredar prioridades del padre
  set_priority(child_c, get_priority(context));
  set_prev_priority(child_c, get_prev_priority(context));
  ...
  *(parent_regs + REG_A0) = get_id(child_c); // return for parent is child's id
  ...
  if (helper_find_context(child_c, used_contexts) == (uint64_t*)0) {
      helper_use_context(child_c);
  }
...
void implement_lock(uint64_t *context)
{
  ...
  if (LOCK_OWNER == DEFAULT_LOCK_OWNER)
  {
    ...
    //Lock ocupao
    lock_holder=find_context_by_id(LOCK_OWNER);
    if(lock_holder!=(uint64_t*)0){
      donate_priority(context,lock_holder);
    }
    deadlock_count=deadlock_count+1;
    //yield mediante manipulacion de prioridad
    if(deadlock_count>10){
      deadlock_count=0;
      if (get_prev_priority(context) == 0) {
          current_priority = get_priority(context);
          set_prev_priority(context, current_priority);
          if (current_priority > 1) {
            set_priority(context, current_priority - 1);
            sort_context();
          }
        }
    }
...
uint64_t load_virtual_memory(uint64_t *table, uint64_t vaddr)
{
  ...
  // update LRU on memory access
  if (frame != 0)
    update_lru_entry(current_context, page, frame);
...
void init_lru(uint64_t max_pages)
{
  lru_max_size = max_pages;
  lru_pages = zmalloc(max_pages * sizeof(uint64_t));
  lru_frames = zmalloc(max_pages * sizeof(uint64_t));
  lru_contexts = zmalloc(max_pages * sizeof(uint64_t));
  lru_size = 0;
  lru_clock = 0;
}


// Encuentro el indice de una entrada LRU
uint64_t find_lru_entry(uint64_t *context, uint64_t page)
{
  uint64_t i;
  i = 0;
  while (i < lru_size) {
    if (*(lru_pages + i) == page)
      if (*(lru_contexts + i) == (uint64_t)context)
        return i;
    i = i + 1;
  }
...
void update_lru_entry(uint64_t *context, uint64_t page, uint64_t frame)
{
  ...
  entry = find_lru_entry(context, page);
  
  if (entry < lru_size) {
    i = entry;
    while (i > 0) {
      *(lru_pages + i) = *(lru_pages + i - 1);
      *(lru_frames + i) = *(lru_frames + i - 1);
      *(lru_contexts + i) = *(lru_contexts + i - 1);
      i = i - 1;
    }
    *(lru_pages + 0) = page;
    *(lru_frames + 0) = frame;
    *(lru_contexts + 0) = (uint64_t)context;
  } else {
    if (lru_size < lru_max_size) {
      i = lru_size;
      while (i > 0) {
        *(lru_pages + i) = *(lru_pages + i - 1);
        *(lru_frames + i) = *(lru_frames + i - 1);
...
uint64_t get_lru_victim_frame(uint64_t *victim_context, uint64_t *victim_page)
{
  ...
  *victim_page = *(lru_pages + lru_size - 1);
  victim_frame = *(lru_frames + lru_size - 1);
  *victim_context = *(lru_contexts + lru_size - 1);
...
uint64_t handle_page_fault(uint64_t *context)
{
  ...
  if (pavailable())
  {
    ...
    update_lru_entry(context, page, frame);
  ...
  else
  {
    ...
    if (victim_context != 0)
    {
      unmap_page((uint64_t *)victim_context, victim_page);
    }
    
    map_page(context, page, victim_frame);
    
    update_lru_entry(context, page, victim_frame);
